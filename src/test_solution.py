...
def test_get_single_member_implemented(client):
    response = client.get('/member/2020')
    assert response.status_code == 200
...
@pytest.mark.it("Method GET /member/2020 should return Thiago")
def test_get_first_member_Thiago(client):
    response = client.get('/member/2020')
    data = json.loads(response.data)
    assert data is not None
    assert "first_name" in data
    assert data["first_name"] == "Thiago"

@pytest.mark.it("Implement method DELETE /member/<int:id> to delete a family member")
def test_delete_member(client):
    response = client.delete('/member/2020')
    assert response.status_code == 200

@pytest.mark.it("Method DELETE /member/2020 should return dictionary with 'done' key")
...
@pytest.mark.it("After deleting the member 2020 we called GET /members and it should return a list with 4 members")
